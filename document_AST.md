...
混在バインディングが検証不能になるかどうかは**ASTごとに違い、事前には分からない**。
実測すると、`[ge, take_first]` では混在の58%が検証不能、`[multiple_of, mul_k]` では100%が
検証不能である一方、**`[gt, ge]` では0%**——どの混在も一様版で再現できてしまう。

```
gt@a → ge@b   は  x > a かつ x >= b。整数なので x >= b は x > b-1 と同じ
              まとめて  x > max(a, b-1)
gt@k → ge@k   は  x > k
a・b は値域 2〜10 なので max(a, b-1) も 2〜10 に収まり、常に k として存在する
```

つまり安全なASTも実在する。しかし安全性を個別に判定するにはバインディング空間の全探索が要り、
それを267,744件について毎回行うのは現実的でない。**「全スロット同一値」という一律の規則は、
その判定を不要にするための選択である。**（`tests/test_literal_binding_equivalence.py` が
両方の実例を固定している）
