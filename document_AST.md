...
#### 単一opの識別だけでは足りない（実測で判明）

上の条件を満たす70件の入力集合で、**difficulty 3 に偽の衝突が30件あった。**
固定入力集合の上では出力列が一致するのに、集合の外の入力では食い違うASTの組である。
`['add_k','abs','zero']` と `['add_k','every_other','zero']` などで、いずれも
`zero` のように「特定の値を踏まないと差が出ない」opが絡む。

偽の衝突があると、L536 の漏洩検査で**重複でないテスト側レコードが除外される**。
漏洩する方向ではないので危険ではないが、テスト集合が理由なく削れる。
...
第2段は反例駆動で満たす。食い違いを見つけたら、**それを分けた入力そのもの**を入力集合に足す。
実測では 70件 → 82件（反例12件、3巡）で収束した。

```
偽の衝突（difficulty 1〜3 の 12,720件 × 外部入力 5,000件）
  反例駆動の前  30 件
  反例駆動の後   0 件
```

**difficulty 4 は未検証である。**255,024件あり全数検証が現実的でない。したがってこれは
経験的な基準であって証明ではなく、「この範囲では区別できた」までしか言えない。
残る偽の衝突は分割後の漏洩検査で過剰除外として現れるので、その件数を報告すること。

なお `always_empty` は誤判定していない。difficulty 3 で「常に空」と判定された1,572件は、
外部入力5,000件でも全件が本当に常に空だった。危険な方向（実際は空でないのに空と記録する）
には倒れていない。
